	...
	**/
	class Controller
	{
		...
		**/
		 ...
		 * @access  private
		**/
		private $moduleExt = '.inc.php';
		
		/**
		 * @var	 string  extension of module
		 * @access  private
		**/
		private $templateExt = '.tpl.html';
		...
		**/
		public function doModel()
		{
			$this->moduleHandler();
			if ($_POST) {
				$this->module->processPostData($this->templateEngine);
			} else {
				$this->module->setStartVars($this->templateEngine);
			}
			...
			if ($action = $this->getCurrentAction()) {
				while (true) {
					...
					if (is_file($candidate . $this->templateExt)) {
						$this->template = array_pop($action);
						break;
