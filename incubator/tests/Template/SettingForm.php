...
/* $Id$ */
	...
	**/
	...
	class SettingForm_Test extends PHPUnit2_Framework_TestCase
	{
        public $setting;
        
        function setUP()
        {
            $this->setting = SettingForm::factory();
		...
		function test_Form()
		{
            import('Form');
            $form = new Form();
            $this->setting->setForm($form->add(Primitive::string('test1')->
						   setDefault('')->
                           setValue('<test>')->
						   setRequired(true))->
			...
			$this->assertEquals('', $this->setting->getValue('test2'));
		}

	}

	$suite = new PHPUnit2_Framework_TestSuite('SettingForm_Test');
	$result = $suite->run();
	$message = '';
