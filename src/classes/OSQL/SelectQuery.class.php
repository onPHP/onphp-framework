	...
	final class SelectQuery extends SelectQuerySkeleton
	{
		...
		public function toString(DB $db)
		{
			...
			for ($i = 0; $i < sizeof($this->from); $i++) {
				if ($i == 0)
					$separator = '';
				elseif ($this->from[$i] instanceof FromTable)
					$separator = ', ';
				else
					$separator = ' ';
				$fromString .= $separator.$this->from[$i]->toString($db);
			}
