...
use OnPHP\Main\OQL\Statement\OqlQuery;
...
{	
	public function testEmpty()
	{
		...
		foreach ($clauses as $clauseName) {
			try {
				call_user_func(array(OQL::class, $clauseName), null);
				$this->fail();
		...
		foreach ($clauses as $clauseName) {
			try {
				call_user_func(array(OQL::class, $clauseName), '');
				$this->fail();
		...
		// Without assertion this test will be marked as risky ;-)
		$this->assertEquals(count($clauses)*2, $fails);
	}

	public function testProperties()
	{
		$query = OQL::select('from '.TestUser::class);
		...
		$this->assertCriteria(
			OQL::select('from '.TestUser::class)->
				addProperties(
